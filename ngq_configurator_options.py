...
class AboutPagesManagerOption(QWidget, Ui_AboutPagesManagerForm):
    ...
    def addPage(self):
        ...
        if dlg.exec_():
            ...
            self.saveValue()
            Log(u"Add new about page: {0}".format(values), u'debug')

    def editPage(self):
        lwi = self.listWidget.currentItem()
        dlg = AboutPageSettings(
            (
                unicode(lwi.data(self.DATA_ROLE_PAGE_NAME_EN).toPyObject().toUtf8(), encoding="UTF-8"),
                unicode(lwi.data(self.DATA_ROLE_PAGE_NAME_TR).toPyObject().toUtf8(), encoding="UTF-8"),
                unicode(lwi.data(self.DATA_ROLE_PAGE_FILE).toPyObject().toUtf8(), encoding="UTF-8")
            ),
            self)
        ...
        if dlg.exec_():
            ...
            self.saveValue()
            Log(u"Edit about page: {0}".format(values), u'debug')
    ...
    def saveValue(self):
        ...
        for i in range(self.listWidget.count()):
            ...
            if lwi.data(self.DATA_ROLE_STATE).toPyObject() is False:
                page_config[u'name'] = [
                    unicode(lwi.data(self.DATA_ROLE_PAGE_NAME_EN).toPyObject().toUtf8(), encoding="UTF-8"),
                    unicode(lwi.data(self.DATA_ROLE_PAGE_NAME_TR).toPyObject().toUtf8(), encoding="UTF-8")
                ]
                page_config[u'content_file'] = unicode(lwi.data(self.DATA_ROLE_PAGE_FILE).toPyObject().toUtf8(), encoding="UTF-8")
