...
class QGISOptions(QWidget):
    ...
    def __showOptionsSettings(self):
        ...
        for option_set_name in self.__option.value.keys():
            ...
            item.setData(Qt.DisplayRole, "%s [%s]"%(option_set_name, self.__option.value[option_set_name]))
        ...
        if self.qlw.count() == self.__max_sets_count:
            ...
            self.pbAddOptionsSet.setText("%s (%s:%d)"%(self.tr("Add"), self.tr("max"), self.__max_sets_count))
