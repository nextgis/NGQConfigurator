...
class JenkinsJobWorker(QObject):
    ...
    finished = pyqtSignal()
    def __init__(self, check_time = 10000000, done_builds_max_count = 2, parent = None):
        QObject.__init__(self, parent)
...
class JenkinsJobModel(BuildSystemModel):       
    def __init__(self, parent = None):
        ...
        self.__j_worker.foundBuildInBuilding.connect(self.__inBuildingBuildProcess)
        ...
        if build_number == -1:
            if len(self._getBuildsByState(self.IN_QUEUE_BUILD_STATE)) != 0:
                for build in self._getBuildsByState(self.IN_QUEUE_BUILD_STATE):
                    self._removeBuild(build.getNumber(), self.IN_QUEUE_BUILD_STATE)
        else:
    ...
    def __inBuildingBuildProcess(self, build_info):
        ...
        if build_number == -1:
            if len(self._getBuildsByState(self.IN_PROCESS_BUILD_STATE)) != 0:
                for build in self._getBuildsByState(self.IN_PROCESS_BUILD_STATE):
                    self._removeBuild(build.getNumber(), self.IN_PROCESS_BUILD_STATE)
        else:
            state_id = self._getStateBuidByNumber(build_number)
            if state_id == None:
                self._addBuild( Build(**build_info), self.IN_PROCESS_BUILD_STATE )
